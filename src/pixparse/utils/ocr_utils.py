...
def get_ocr_metrics(model, tokenizer, image_input, text_input, device_env, max_recursion_length):
    """
...
def generate_ocr(model, tokenizer, encoder_outputs: torch.FloatTensor, device_env, max_recursion_length) -> List[str]:
    """
    ...
    with torch.inference_mode():
        ...
        generated_tokens = get_generated_tokens(model, tokenizer, encoder_outputs, device_env, max_recursion_length)
        generated_texts = [tokenizer.trunk.decode(text) for text in generated_tokens.tolist()]
    return generated_texts

def get_generated_tokens(model, tokenizer, encoder_outputs, device_env, max_recursion_length):
    """
    # TODO This "hacky" function should eventually be replaced by .generate() from GenerationMixin that does the same thing.
    """
    input_ids = torch.full((encoder_outputs.shape[0], 1), tokenizer.trunk.cls_token_id).to(device_env.device)
        
    finished_samples = torch.zeros(input_ids.shape[0], dtype=torch.bool).to(device_env.device)
